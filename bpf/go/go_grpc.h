...
static __always_inline void grpc_server_handleStream_start(struct pt_regs *ctx) {
    bpf_dbg_printk("=== grpc uprobe/server_handleStream === ");
...
static __always_inline int grpc_server_handleStream_end(struct pt_regs *ctx) {
    bpf_dbg_printk("=== grpc uprobe/server_handleStream return === ");

    void *goroutine_addr = GOROUTINE_PTR(ctx);
    bpf_dbg_printk("goroutine_addr %lx", goroutine_addr);

    grpc_srv_func_invocation_t *invocation = (grpc_srv_func_invocation_t *)bpf_map_lookup_elem(&ongoing_grpc_server_requests, &goroutine_addr);
    ...
    u16 *status = (u16 *)bpf_map_lookup_elem(&ongoing_grpc_request_status, &goroutine_addr);
    ...
    http_request_trace *trace = (http_request_trace *)bpf_ringbuf_reserve(&events, sizeof(http_request_trace), 0);
    if (!trace) {
    ...
    goroutine_metadata *g_metadata = (goroutine_metadata *)bpf_map_lookup_elem(&ongoing_goroutines, &goroutine_addr);
    if (g_metadata) {
...
static __always_inline void grpc_transport_writeStatus_start(struct pt_regs *ctx) {
    bpf_dbg_printk("=== grpc uprobe/transport_writeStatus === ");
...
static __always_inline void grpc_ClientConn_Invoke_start(struct pt_regs *ctx) {
...
static __always_inline void grpc_ClientConn_NewStream_start(struct pt_regs *ctx) {
...
static __always_inline void grpc_ClientConn_Close_start(struct pt_regs *ctx) {
...
static __always_inline int grpc_ClientConn_Invoke_end(struct pt_regs *ctx) {
    bpf_dbg_printk("=== uprobe/proc grpc ClientConn.Invoke/ClientConn.NewStream return === ");

    void *goroutine_addr = GOROUTINE_PTR(ctx);
    bpf_dbg_printk("goroutine_addr %lx", goroutine_addr);

    grpc_client_func_invocation_t *invocation = (grpc_client_func_invocation_t *)bpf_map_lookup_elem(&ongoing_grpc_client_requests, &goroutine_addr);
    bpf_map_delete_elem(&ongoing_grpc_client_requests, &goroutine_addr);
...
static __always_inline void grpc_transport_loopyWriter_writeHeader_start(struct pt_regs *ctx) {
    bpf_dbg_printk("=== grpc uprobe/proc transport.(*loopyWriter).writeHeader === ");
    ...
    if (stream_id) {
        grpc_client_func_invocation_t *invocation = (grpc_client_func_invocation_t *)bpf_map_lookup_elem(&ongoing_streams, &stream_id);
...
static __always_inline void grpc_transport_loopyWriter_writeHeader_end(struct pt_regs *ctx) {
    bpf_dbg_printk("=== grpc uprobe/proc transport.(*loopyWriter).writeHeader returns === ");
    void *goroutine_addr = GOROUTINE_PTR(ctx);
...
static __always_inline int grpc_hpack_Encoder_WriteField_start(struct pt_regs *ctx) {
    ...
    grpc_client_func_invocation_t *invocation = (grpc_client_func_invocation_t *)bpf_map_lookup_elem(&ongoing_grpc_header_writes, &goroutine_addr);
