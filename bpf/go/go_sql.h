...
static __always_inline void sql_queryDC_start(struct pt_regs *ctx) {
    bpf_dbg_printk("=== sql uprobe/queryDC === ");
...
static __always_inline int sql_queryDC_end(struct pt_regs *ctx) {
    bpf_dbg_printk("=== uprobe/queryDC return === ");
    void *goroutine_addr = GOROUTINE_PTR(ctx);
    bpf_dbg_printk("goroutine_addr %lx", goroutine_addr);

    sql_func_invocation_t *invocation = (sql_func_invocation_t *)bpf_map_lookup_elem(&ongoing_sql_queries, &goroutine_addr);
    if (invocation == NULL) {
    ...
    sql_request_trace *trace = (sql_request_trace *)bpf_ringbuf_reserve(&events, sizeof(sql_request_trace), 0);
    if (trace) {
        task_pid(&trace->pid);
