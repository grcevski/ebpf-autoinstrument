...
func (pt *ProcessTracer) AllowPID(pid uint32, fi *exec.FileInfo) {
	for i := range pt.Programs {
		pt.Programs[i].AllowPID(pid, fi)
	}
}

func (pt *ProcessTracer) BlockPID(pid uint32, fi *exec.FileInfo) {
	for i := range pt.Programs {
		pt.Programs[i].BlockPID(pid, fi)
	}
}
